...
# roomGraph={0: [(1, 1), {'n': 1}], 1: [(1, 2), {'s': 0, 'n': 2}], 2: [(1, 3), {'s': 1}]}
...
while len(graph) < len(roomGraph):
    ...
    #Checks if the current room the player is in. Is in the 'graph' dictianary.
    if currentRoom not in graph:

        #add current room to dictionary
        graph[currentRoom] = {}

        #loop through current rooms exits
        for exit in player.currentRoom.getExits():
            print(f'exit {exit}')

            #applies all exits with "?"
            graph[currentRoom][exit] = "?"

    # loop through the path of currentRooms ID
    for path in graph[currentRoom]:

        print(f'path direction {path}')
        ...
        if graph[currentRoom][path] == "?":
            ...
            print(f'Pathway taken {pathway}')

            if pathway is not None:

                traversalPath.append(pathway)

                player.travel(pathway)

                new_room = player.currentRoom.id
                print(f'new rooms ID {new_room}')
                ...
                if new_room not in graph:
                    ...
                    for exit in player.currentRoom.getExits():
                        ...
                        print(f'exits of new room {exit}')


            graph[currentRoom][pathway] = new_room
            # print(f'{new_room} the new room')

            graph[new_room][reverse_direction(pathway)] = currentRoom
            # print(f'{currentRoom} reverse')


            currentRoom = new_room
            # print(f'{new_room} assignment')
